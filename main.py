...
# voiceover_filepath = openai_wrapper.generate_speech(voice_text, file_path=f"audio/{top_topic}-{bottom_topic}.mp3")
...
txt_clip_top = TextClip(top_topic, fontsize=70, color='white', stroke_color='black', stroke_width=2, font='Verdana').set_duration(video_duration).set_position(("center", 50)).set_start(top_start_time)
txt_clip_bottom = TextClip(bottom_topic, fontsize=70, color='white', stroke_color='black', stroke_width=2, font='Verdana', method='caption', align='center').set_duration(video_duration).set_position(("center", 1750)).set_start(bottom_start_time)
